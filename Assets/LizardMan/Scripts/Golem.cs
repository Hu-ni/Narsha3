...
public class Golem : Character
{
    ...
    IEnumerator UseE()
    {
        ...
        while (!SkillE.GetComponent<Active>().isActive)
        {
            ...
            {
                if (!Mathf.Approximately(agent.speed, SPEED()) && animator.GetCurrentAnimatorClipInfo(0)[0].clip.name.Contains("jump") && animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 0.97f)
                {
                    float damage;
                    ...
                    foreach (Collider coll in colls)
                    {
                        ...
                        {
                            Debug.Log(coll.name);
                            coll.GetComponent<Character>().Stun(3);
